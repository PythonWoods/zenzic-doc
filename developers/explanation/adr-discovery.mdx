...
(`check`, `guard`, `score`, `diff`, `clean`) retains the strict default
(`fallback_to_cwd=False`) and will continue to fail loudly outside a project
scan scope.

```python
# src/zenzic/cli/_standalone.py — the only permitted call site for fallback_to_cwd=True
def init(path=None, plugin=None, force=False):
    repo_root = find_repo_root(fallback_to_cwd=True)  # Genesis Fallback
    ...

# Every other command — strict scan scope enforcement
