...
However, before this decision, Zenzic had no native mechanism to **verify** that the committed
...
The invariant: if the `<!-- zenzic:badge -->` marker is absent from a file, `--check-stamp`
...
`_check_stamp_file(path, expected_url)` reads the file from disk, locates the `<!-- zenzic:badge -->`
...
- `_check_stamp_file(path, expected_url)` returns **True (pass)** when:
  - The file does not exist.
  - The `<!-- zenzic:badge -->` marker is absent.
...
- [ADR 009: Path Sovereignty](./adr-path-sovereignty.mdx) — same principle applied to config path resolution.
- [ADR 005: Agnostic Universalism](./adr-agnostic-universalism.mdx) — portability as a first-class design constraint.
