...
In the implementation prior to current release, the coordinator collected results by
...
**Behavioural changes in current release:**
...
`_worker()` and `_scan_single_file()` are **unchanged** in current release Given the
