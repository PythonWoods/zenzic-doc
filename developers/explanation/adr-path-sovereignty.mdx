...
**Date:** 2026-04-12 (v0.7.0 sprint, CEO-052)
...
ADR-007 (Sovereign Sandbox) established that the **perimeter** follows the
...
- Contributors implementing new CLI commands that accept a `PATH` argument

  **must** call `find_repo_root(search_from=resolved_path)` and invoke
  `_apply_target()`. This is now a documented invariant in the contribution guide.

- The `fallback_to_cwd=True` parameter of `find_repo_root()` is reserved

  exclusively for the `init` command (Genesis Fallback — see ADR-003). No other
  command may use it.
