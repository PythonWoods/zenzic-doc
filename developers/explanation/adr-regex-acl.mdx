...
**Decider:** Tech Lead
**Date:** 2026-05-10 (current release)
...
Where legacy patterns used stdlib-only constructs such as lookbehind,
...
2. **All production regex imports go through the ACL.** Core modules must use
   `from zenzic.core import regex as re` instead of importing `re2` directly.
...
- [ADR 001: Lint the Source](./adr-lint-source.mdx) — content and source
  semantics must stay readable to humans.
- [ADR 002: Zero Subprocesses Policy](./adr-zero-subprocesses.mdx) — the regex
  layer must remain in-process and deterministic.
- [ADR 011: Cross-Instance Absolute Path Allowlist](./adr-cross-instance-allowlist.mdx)
  — another example of using a narrow architectural boundary to absorb an
  engine-specific mismatch without polluting source content.
