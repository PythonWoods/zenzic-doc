...
> keep the document content unchanged and add a temporary exclusion in
> `.zenzic.toml` with a clear review comment. This avoids masking content drift
> while still keeping the guard usable on unstable networks.
