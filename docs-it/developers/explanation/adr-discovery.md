...
**Risoluzione:** `find_repo_root()` acquisisce un parametro keyword-only `fallback_to_cwd` (predefinito `False`). Quando `fallback_to_cwd=True` e nessun marcatore di radice viene trovato, la funzione restituisce `Path.cwd()` invece di sollevare un'eccezione. Questo è chiamato **Fallback Genesis**.

Per le specifiche del parametro, vedere [API Reference — `find_repo_root`](../reference/adapter-api.md#find_repo_root-fallback_to_cwd-bool--false---path).

**Ambito di autorizzazione:** Il Fallback Genesis è un'esenzione a punto singolo. Solo il comando `init` passa `fallback_to_cwd=True` (in `src/zenzic/cli/_standalone.py`). Ogni altro comando (`check`, `guard`, `score`, `diff`, `clean`) mantiene il default rigoroso (`fallback_to_cwd=False`) e continuerà a fallire esplicitamente fuori da un perimetro di progetto.
...
- `src/zenzic/cli/_standalone.py` — comando `init`, unico consumatore di `fallback_to_cwd=True`
