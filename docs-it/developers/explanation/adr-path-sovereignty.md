...
- Eseguire Zenzic da qualsiasi directory produce risultati identici a
  eseguirlo dall'interno del repository target — nessuna sorpresa per gli
  operatori CI.

- Il parametro `fallback_to_cwd=True` di `find_repo_root()` è un percorso di

    fallback vincolato. Usalo solo dove la semantica del comando richiede
    esplicitamente un fallback alla directory corrente, evitando di introdurlo
    come default nei flussi di comando PATH-targeted.
