...
| Modalità cartella (Folder) | `docs/` + `docs-<locale>/` |
...
Le Ghost Route sono pagine generate dal motore durante la compilazione (elenchi di tag, indici paginati, pagine degli autori) che non hanno un file sorgente Markdown fisico sul disco. Un controllo sul filesystem le segnalerà sempre come interrotte.
...
- Non analizzare mai `mkdocs.yml`, `zensical.toml` o qualsiasi altro file di configurazione del motore direttamente all'interno di un controllo. Questa responsabilità appartiene esclusivamente all'adapter.
