...
# Guida di Stile Zenzic

> *"Il rigore delZenzic nel codice deve estendersi a ogni pixel che l'utente vede."*

Questo documento codifica il **Zenzic Visual Language** — le regole
...
- <Icon name="play" /> &nbsp; **Guida Utente**
    ...
    [<Icon name="arrow-right" /> Esplora la Guida](../../../how-to/install.md)
```
...
**Card di presentazione** (es. demo "Zenzic in Action" nella homepage)
...
| `:::info` | **Output Zenzic** | Blocchi di output CLI e campioni di report Zenzic |
...
| `:::note` | **Filosofia** | Visione del progetto, manifesto di design, credo delZenzic |
...
1. È un heading **H2 o H3** (mai H1 — alcuni motori generano automaticamente gli ID H1 dall'etichetta della barra laterale).
...
Ogni file sorgente documentale (`.md`, `.mdx` e contenuti equivalenti)
deve includere metadati SPDX.
...
### Regola delle Modifiche Significative (MUST)

Per modifiche significative (nuova logica, blocchi contenutistici rilevanti,
riscritture strutturali), i contributori **devono** aggiungere la propria riga
`SPDX-FileCopyrightText` sotto quella del progetto.

```html
<!-- SPDX-FileCopyrightText: 2026 PythonWoods <dev@pythonwoods.dev> -->
<!-- SPDX-FileCopyrightText: 2026 Nome Contributore <contributor@example.com> -->
<!-- SPDX-License-Identifier: Apache-2.0 -->
```

Per correzioni banali (typo, punteggiatura, sola formattazione) non è
richiesta una nuova riga contributore.

### Modello di Governance Legale

Zenzic **non** richiede un modello CLA con trasferimento dei diritti.
La governance si basa su:

- **DCO** (Developer Certificate of Origin) per l'attestazione di paternità.
- **REUSE/SPDX** per la tracciabilità copyright/licenza per file.

I contributori mantengono il copyright sulle modifiche significative e
dichiarano la paternità tramite header SPDX.
...
- [ ] Nessun literal esadecimale (`#rrggbb`) in `src/` al di fuori di `ZenzicPalette._*` (§9).
- [ ] Tutti i riferimenti ai colori usano `ZenzicPalette.*` — nessuna costante piatta rimossa (§9).

---

## 8. Gateway ZenzicUI {#zenzicui-gateway}

Tutto l'output terminale brandizzato di Zenzic fluisce attraverso un unico oggetto: `ZenzicUI` in
`src/zenzic/ui.py`. I moduli di comando non devono **mai** istanziare `Console` o `ZenzicUI`
...
| Metodo | Quando usare |
| :--- | :--- |
| `print_header(version)` | Il banner Zenzic Frame ad inizio output — una volta per invocazione del comando |
...
`Console` o `ZenzicUI` creata localmente sarà congelata prima che il flag abbia effetto,
...
- [ ] Nessuna istanziazione `Console(...)` o `ZenzicUI(...)` nei moduli di comando.
...
`ZenzicPalette` in `src/zenzic/ui.py` è la **sola fonte autorizzata di valori colore**
nell'intera codebase Zenzic. Questa è la Zero Hex Law.
...
!!! warning "Vincolo di Design"
    ...
    possono comparire in `src/` **eccetto** all'interno degli attributi privati `ZenzicPalette._*`.
...
| :--- | :---: | :--- |
| `ZenzicPalette.BRAND` | `#4f46e5` | Colore primario / brand Zenzic (Indigo) |
| `ZenzicPalette.SUCCESS` | `#10b981` | OK · pulito · passato (Emerald) |
| `ZenzicPalette.WARNING` | `#f59e0b` | Attenzione · advisory (Amber) |
| `ZenzicPalette.ERROR` | `#f43f5e` | Errore · link rotti (Rose) |
| `ZenzicPalette.DIM` | `#64748b` | Testo muto · secondario (Slate) |
...
```python
# CORRETTO — alias semantico tramite ZenzicPalette
...
```python
# VIETATO — literal hex al di fuori di ZenzicPalette
text = Text.from_markup("[#4f46e5]Zenzic[/]")   # ✗

# VIETATO — import di costante piatta (rimossa in)
from zenzic.ui import INDIGO, EMERALD            # ✗

# VIETATO — alias locale
P = ZenzicPalette                              # ✗  usare sempre il nome completo della classe
```

### Aggiornare la palette

Per cambiare un colore, modificare **solo** l'attributo esadecimale `_PRIVATO` corrispondente
all'interno di `ZenzicPalette` in `src/zenzic/ui.py`. Tutti gli alias semantici e le
...
- [ ] Nessun literal esadecimale (`#rrggbb`) in `src/` al di fuori di `ZenzicPalette._*`.
- [ ] Nessun nome colore Rich grezzo (`"red"`, `"cyan"`) per uso della palette brand — usare `ZenzicPalette.*`.
- [ ] Nessun alias locale `P = ZenzicPalette` — usare sempre il nome completo della classe.
- [ ] Nessun `from zenzic.ui import INDIGO` (o qualsiasi costante piatta rimossa).

---

## 10. Legge della Componentizzazione degli Asset MDX {#mdx-asset-componentization}

**Direttiva:** ZRT-DOC-010

!!! warning "Vincolo di Design"

    Qualsiasi asset vettoriale destinato all'uso **esclusivo all'interno delle pagine MDX** deve essere implementato come componente React (`.tsx`), mai come file `.svg` statico.

Per il razionale architetturale dettagliato dietro questa direttiva, vedere [Razionale della Componentizzazione degli Asset MDX](../explanation/mdx-asset-rationale.md).
...
- [ ] Nessun nuovo file `.svg` aggiunto a `static/assets/terminal/` (usare un componente `.tsx`).
- [ ] Qualsiasi SVG contenente testo introdotto all'interno di una pagina MDX è implementato come `.tsx`.
