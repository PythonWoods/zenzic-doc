...
sidebar_position: 2
title: Architettura
description: Approfondimento tecnico sulla pipeline a tre fasi di Zenzic, il credential scanner, il protocollo Adapter, il Layered Exclusion Manager e il contratto dei codici di uscita.
---
...
Questa pagina descrive il design interno di Zenzic per contributor e utenti avanzati che devono capire come funziona lo strumento internamente. Per configurazione e uso, consulta [Configuration Reference](../reference/configuration-reference) e [Checks Reference](../reference/checks).

## Integrità Oltre il Codice {#integrity-beyond-code}
...
esecuzione non è deterministico, i risultati dell'analisi non sono affidabili.
...
Questo è l'equivalente infrastrutturale dell'analisi statica: vincolare gli
input di esecuzione, preservare la riproducibilità e rendere auditabile
l'evidenza di sicurezza per contributor e utenti finali.

---

## Pipeline a Tre Fasi {#three-phase-pipeline}
...
class RouteMetadata:
    ...
    version: str | None = None # Etichetta versione opzionale (per supporto futuro)
```
...
Zenzic, la VSM è **consapevole delle versioni**. Per gli adapter che supportano la documentazione multi-versione, il VSM builder:
...
Gli adapter che supportano override `slug` nel frontmatter mappano gli slug nella Virtual Site Map per la validazione della **raggiungibilità**: una pagina con `slug: /quick-start` all'URL `/docs/quick-start` viene correttamente marcata `REACHABLE` anche se il suo percorso file è `docs/guides/getting-started.mdx`.
...
La factory `get_adapter()` interroga il gruppo entry-point `zenzic.adapters` e ricade su `StandaloneAdapter` se non trova un adapter per il motore richiesto. Gli adapter di terze parti si registrano via `pyproject.toml` — vedi la [guida all'implementazione degli adapter](/developers/how-to/implement-adapter).
...
## Complessità Algoritmica {#algorithmic-complexity}

L'architettura di Zenzic separa gli intenti computazionali applicando limiti algoritmici specifici a ciascun dominio:

- **Topologia (Knowledge Graph)**: La validazione dei link esegue una DFS iterativa sul grafo di adiacenza della Virtual Site Map. Con una rappresentazione tramite liste di adiacenza, la complessità della traversata è $\Theta(V+E)$, dove $V$ rappresenta pagine e asset ed $E$ i collegamenti. I registri di ciclicità utilizzano hash set per fornire lookup medi $O(1)$ nelle passate successive.

- **Scansione Semantica**: Il credential scanner e le regole personalizzate utilizzano il motore regex `google-re2`, che evita il backtracking catastrofico tipico di alcuni motori regex tradizionali. Ciò garantisce una complessità lineare $O(N)$ rispetto all'input analizzato ed elimina le vulnerabilità ReDoS basate sul backtracking esponenziale.

- **I/O Discovery**: L'ingestione dei file mantiene una complessità $O(N)$ rispetto al volume totale dei dati elaborati. Quando viene raggiunta una determinata soglia operativa, l'elaborazione può essere distribuita tramite process pool paralleli per ridurre il wall-time, mantenendo invariata la complessità computazionale complessiva.
...
> Vedi [Integrazione CI/CD](../how-to/configure-ci-cd.md) per esempi di workflow.
...
| Migrazione da MkDocs a Zensical | Usa `zenzic check all` con `engine = "mkdocs"` sulla sorgente, `engine = "zensical"` sulla destinazione |

> Vedi [Integrazione CI/CD](../how-to/configure-ci-cd.md) per workflow passo dopo passo corrispondenti a ogni scenario.
...
Zenzic gestisce questo attraverso il blocco di configurazione [`[governance]`](../reference/configuration-reference.md) e il finding [Z601 BRAND_OBSOLESCENCE](../reference/finding-codes.md#z601). La soppressione per-riga format-aware è disponibile per i riferimenti storici intenzionali utilizzando la sintassi di commento appropriata per il tipo di file (`.md` vs. `.mdx`).
