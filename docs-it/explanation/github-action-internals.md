...
description: "Come zenzic-action applica la sicurezza: protocollo path traversal guard, Contratto Exit Code, cascata Root-First Discovery e Intenzione Sovrana."
---
...
> Per il riferimento completo all'input `guard-scan` e gli esempi di workflow, vedi [Riferimento Zenzic GitHub Action — Input](../reference/zenzic-action.md#inputs).

!!! note "Lo scan di guardia è sempre fatale"
...
| [Architettura](./architecture) | Pipeline a due passate di Zenzic Core, credential scanner middleware, protocollo adapter |
| [Decisioni Architetturali](https://zenzic.dev/it/developers/explanation/adr-vault) | Decisioni architetturali dietro il contratto exit code e il path traversal guard |
