...
description: "Integrità dei riferimenti, rilevamento credenziali e uso programmatico."
---
...
Riferimento approfondito sulla Three-Pass Pipeline, credential scanner, controlli di accessibilità
e utilizzo programmatico da Python.

---

## Integrità dei riferimenti {#reference-integrity-v020}
...
| :---: | :--- | :--- |
| 1 | **Harvest** | Legge il file riga per riga; registra tutte le definizioni `[id]: url` in una `ReferenceMap`; esegue il credential scanner su ogni URL e riga |
...
Il Pass 2 inizia solo quando il Pass 1 termina senza security finding. Se il credential scanner scatta
...
Il credential scanner gira **dentro il Pass 1** — ogni URL estratto da una reference definition viene
scansionato nel momento in cui l'harvester lo incontra, prima che qualsiasi altra elaborazione
continui. Il credential scanner applica anche un pass di difesa in profondità alle righe non-definizione
...
| `github-token` | `(?i)\b(?:ghp\|gho\|ghu\|ghs\|ghr)_[a-zA-Z0-9_.-]+\b` | Token GitHub personal, OAuth, app e refresh |
| `gitlab-pat` | `glpat-[A-Za-z0-9\-_]{20,}` | Personal access token GitLab |
...
### Comportamento del credential scanner {#credential-scanner-behaviour}
...
- Il rilevamento è **non sopprimibile** — `--exit-zero`, `exit_zero = true` in `.zenzic.toml` e
  ...
  `--strict` non hanno effetto sui security finding del credential scanner.

- Il codice di uscita 2 è riservato esclusivamente agli eventi credential scanner. Non viene mai usato

  per i fallimenti ordinari dei controlli.

- Il codice di uscita 3 è riservato agli eventi del **path traversal guard** — link che risolvono a
...
- **Isolamento dei link nei blocchi di codice** — lo credential scanner scansiona l'interno dei blocchi
...
!!! note "Scopri il credential scanner in azione"
    ...
    per osservare una violazione credential scanner live e un finding `CIRCULAR_LINK` di tipo info.
...
| Posizione del contenuto | Credential scanner (segreti) | Sintassi snippet | Validazione link/ref |
| :--- | :---: | :---: | :---: |
...
- **Le regole devono essere definite a livello di modulo** — una classe definita in una
  funzione o lambda non può essere pickled e verrà rifiutata al caricamento.

- **Tutti gli attributi di istanza devono essere serializzabili** — i pattern `re.compile()`
  pre-compilati, stringhe e numeri sono sempre sicuri; file handle e closure lambda non lo sono.
...
| **Standalone** (nessuna config engine) | No | Nessun concetto di routing |
...
- **La modalità Standalone salta completamente il controllo orfani** — quando non è presente alcuna

  config del motore di build, ogni file sembrerebbe un orfano. Zenzic salta il controllo
  piuttosto che segnalare rumore.

!!! tip "Forza la modalità Standalone per sopprimere il controllo orfani"
