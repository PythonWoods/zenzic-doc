...
!!! note "Coerenza Fisica — perché i percorsi relativi contano"
    ...
    Alcuni build engine permettono override `slug` nel frontmatter che disaccoppiano l'URL di una pagina dalla sua posizione nel filesystem. Quando questo accade, la "directory padre" per la risoluzione dei link relativi può differire tra il build engine (che risolve dall'URL) e Zenzic (che risolve dal percorso del file).
