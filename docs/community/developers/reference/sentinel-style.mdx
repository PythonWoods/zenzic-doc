...
- [ ] No hex literal (`#rrggbb`) in `src/` outside `SentinelPalette._*` (§9).
- [ ] All colour references use `SentinelPalette.*` — no removed flat constants (§9).

---

## 8. SentinelUI Gateway {#obsidianui-gateway}
...
- [ ] No `Console(...)` or `SentinelUI(...)` instantiation in command modules.
...
## 9. SentinelPalette — Zero Hex Law {#obsidian-palette}
...
- [ ] No hex literal (`#rrggbb`) anywhere in `src/` outside `SentinelPalette._*`.
- [ ] No raw Rich colour names (`"red"`, `"cyan"`) for brand-palette usage — use `SentinelPalette.*`.
- [ ] No local alias `P = SentinelPalette` — always use the full class name.
