...
Zenzic requires Python 3.10 or higher.
...
(Standalone mode). Native adapters for MkDocs, Zensical, and other engines add nav-awareness and i18n support.
...
**What is the credential scanner (exit code 2)?**

Exit code `2` means the credential scanner detected a pattern that looks like a credential
(API key, token, password) in a URL or text. Rotate the credential immediately if you receive
this exit code.

**What is the path traversal guard (exit code 3)?**
...
**The Italian (or any locale) pages show English content even though the URL is `/it/`.**

This is the **i18n Silent Fallback Trap**. Docusaurus derives the folder `path` from
`htmlLang` when `path` is not declared explicitly. With `htmlLang: 'it-IT'`, it looks for
translation files in `i18n/it-IT/` instead of `i18n/it/`. The build completes without
errors and the UI chrome (navbar, breadcrumbs) appears translated — because those strings
live inside Docusaurus's own package — but every content page silently falls back to the
English source.

**Diagnosis:** after `npm run build`, open `.docusaurus/i18n.json` and check whether the
locale entry has `"translate": false`. If so, the path mismatch is the cause.

**Solution:** add `path` explicitly to every locale that uses a regional `htmlLang` tag:
