...
The adapter pattern is the core architectural mechanism that allows Zenzic to validate documentation sites without depending on the runtime of specific build frameworks (e.g., Node.js for Docusaurus).
...
Zenzic adapters fall into two major paradigms: **Engine-Aware** (e.g., `DocusaurusAdapter`) and **Zero-Assumption / Sovereign** (e.g., `StandaloneAdapter`).
...
- **Capabilities:** Enable advanced validation features such as strict orphan detection (Z402), locale shadow detection, and custom URI scheme validation (e.g., Docusaurus `pathname:///` bypasses).
...
> For concrete code comparisons showing how these strategies are implemented in Python, see [Writing an Adapter — Concrete Examples](../how-to/implement-adapter.md#concrete-implementation-examples-docusaurus-vs-standalone).
>
