...
The adapter pattern is the core architectural mechanism that allows Zenzic to validate documentation sites without depending on the runtime of specific build frameworks.
...
Zenzic adapters fall into two major paradigms: **Engine-Aware** (e.g., `ZensicalAdapter`) and **Zero-Assumption / Sovereign** (e.g., `StandaloneAdapter`).
...
- **Capabilities:** Enable advanced validation features such as strict orphan detection (Z402), locale shadow detection, and config-specific asset mapping.
