...
### GAP-002 — Dynamic Navbar Plugin Support

**Category:** Link validation scope
**Status:** Deferred
**Tracked:** GitHub issue (RFC open)
**Related:** —

#### What was deferred

Zenzic's Docusaurus adapter parses `docusaurus.config.ts` statically to build the
Virtual Site Map (VSM). `@docusaurus/plugin-*` navbar entries — items registered
through third-party or first-party Docusaurus plugins — are not parsed. Zenzic
falls back to marking these routes as `REACHABLE`, which means:

- False-positive risk is **low** — plugin-generated routes are rarely broken.
- Miss risk is **non-zero** — genuine orphans hidden behind plugin-only nav entries
  may go undetected in plugin-heavy configurations.

#### Why we deferred it

Plugin navbar items require evaluating JavaScript plugin configuration at parse
time, which would break the **No Subprocesses** invariant (Pillar 2). A stateless
static parser cannot recover plugin-registered routes without executing the plugin.

#### What we will do

The planned resolution introduces:

1. A **structured warning** emitted when plugin nav items are detected, communicating
   the reduced coverage explicitly to the operator.
2. A `dynamic_nav_plugins = true` config key in `.zenzic.toml` that opts the project
   into explicit acknowledgment of this limitation, suppressing the warning.

#### Mitigation

Plugin-heavy Docusaurus projects should ensure that all plugin-registered routes
also appear in at least one non-plugin navigation entry (sidebar, header nav, or
a direct in-page link). Zenzic audits those paths normally.
