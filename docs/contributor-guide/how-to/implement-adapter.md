...
Most engines return `frozenset()`. If your engine defines custom URI schemes
...
To see how these adapter methods are implemented in practice, here is a comparison between the full engine-aware `ZensicalAdapter` and the zero-assumption `StandaloneAdapter`.
...
**`ZensicalAdapter.provides_index()`** — full engine awareness:
...
**Key difference:** In this method, both adapters behave identically by resolving the presence of the `index.md` file, although `ZensicalAdapter` could be extended to support dynamic config-driven index mapping.
...
**`ZensicalAdapter.get_nav_paths()`** — config-driven navigation:

```python
def get_nav_paths(self) -> frozenset[str]:
    # self._nav_paths contains the set of paths parsed from zensical.toml
    return self._nav_paths
```
...
**`ZensicalAdapter._classify_route()`** — classification rules:

```python
def _classify_route(self, rel: Path, nav_paths: frozenset[str]) -> RouteStatus:
    # Rule 1: Private/meta files (starting with _) → IGNORED
    if any(part.startswith("_") for part in rel.parts):
        return "IGNORED"

    # Rule 2: No explicit nav → all files are REACHABLE
    ...
    # Rule 4: File exists but is not discoverable via navigation
...
The contrast is stark. `ZensicalAdapter` implements a priority cascade in `_classify_route()`, which `get_route_info()` calls internally. `StandaloneAdapter` has no `_classify_route()` helper — `get_route_info()` returns `REACHABLE` directly because with no engine there is no navigation contract.
...
```python
def get_link_scheme_bypasses(self) -> frozenset[str]:
    """Zensical projects have no engine-specific link-scheme bypass."""
    return frozenset()
```
...
When a custom link scheme is encountered, the Core checks `adapter.get_link_scheme_bypasses()`. If it is not found in the returned set, it is flagged as an invalid link or absolute path violation.
