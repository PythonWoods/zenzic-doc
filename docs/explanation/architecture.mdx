...
<div className="zz-figure">
  <ThemedImage
    alt="Zenzic Three-Phase Analysis Pipeline: File Set → Pass 1 (credential scanner + Content streams) → Pass 2 (Cross-Check) → Pass 3 (Integrity Report) → Quality Report"
    sources={{
...
- **Core Maintainer**: `ZENZIC_CORE_PATH` set → `uv run --project <path>` against local source.
