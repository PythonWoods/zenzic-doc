...
<div style={{borderRadius:'8px',overflow:'hidden',margin:'1.5rem 0'}}>
  <ThemedImage
    alt="Zenzic Two-Pass Analysis Pipeline: File Set → Pass 1 (Shield + Content streams) → Pass 2 (Cross-Check) → Pass 3 (Integrity Report) → Quality Report"
    sources={{
...
<div style={{borderRadius:'8px',overflow:'hidden',margin:'1.5rem 0'}}>
  <ThemedImage
    alt="Zenzic Shield normalization algorithm: 8-step pipeline from Raw Line through ZRT-006/007/003 normalization passes to dual raw/normalized scan, deduplication, and SecurityFinding output"
    sources={{
...
`zensical` engines. The blue-team fixtures produce an identical Sentinel Seal on all
...
Step 3 — CEO-043 Sandbox Guard
         ...
         docs_root as the new repo_root so Blood Sentinel guards escapes
...
| Sandbox guard | Blood Sentinel perimeter is anchored at `docs_root`, not `cwd` |
...
| `get_ui()` | The current `SentinelUI` singleton (wraps the console) |
...
| Add a shared utility | Add to `_shared.py` and import via `from . import _shared` — never instantiate `Console` or `SentinelUI` locally |

### Visual identity — `zenzic.core.ui` {#core-ui}

`SentinelPalette`, `SentinelUI`, `make_banner`, `emoji`, and `SUPPORTS_COLOR` live in
