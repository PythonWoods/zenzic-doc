...
<div style={{borderRadius:'8px',overflow:'hidden',margin:'1.5rem 0'}}>
  <img src={useBaseUrl('/assets/brand/svg/zenzic-pipeline-flow.svg')} alt="Zenzic Two-Pass Analysis Pipeline: File Set → Pass 1 (Shield + Content streams) → Pass 2 (Cross-Check) → Pass 3 (Integrity Report) → Quality Report" style={{width:'100%',display:'block'}} />
</div>
...
<div style={{borderRadius:'8px',overflow:'hidden',margin:'1.5rem 0'}}>
  <img src={useBaseUrl('/assets/brand/svg/zenzic-shield-normalization.svg')} alt="Zenzic Shield normalization algorithm: 8-step pipeline from Raw Line through ZRT-006/007/003 normalization passes to dual raw/normalized scan, deduplication, and SecurityFinding output" style={{width:'100%',maxWidth:'560px',display:'block',margin:'1.5rem auto'}} />
</div>
...
<div style={{borderRadius:'8px',overflow:'hidden',margin:'1.5rem 0'}}>
  <img src={useBaseUrl('/assets/brand/svg/zenzic-vsm-projection.svg')} alt="Zenzic Virtual Site Map: projection from physical filesystem files through VSMBuilder and Adapter Protocol to a route catalog with REACHABLE, ORPHAN, and GHOST route statuses" style={{width:'100%',display:'block'}} />
</div>
