...
This section documents the security hardening features introduced in v0.6.1. These properties are verified by the test suite and enforced by the `_validate_docs_root` guard and the `safe_read_line` I/O fence.
...
Starting with v0.6.1 "Quartz Clarity", the VSM is **version-aware**. For adapters that support multi-version documentation (currently `DocusaurusAdapter`), the VSM builder:
...
red-team attack vectors produce identical findings across `standalone`, `mkdocs`, and
`zensical` engines. The blue-team fixtures produce an identical Zenzic Audit Badge on all
...
| Sandbox guard | Path traversal guard perimeter is anchored at `docs_root`, not `cwd` |
...
Zenzic v0.7.0 uses exclusively a **DFA (Deterministic Finite Automaton)** engine for all
...
The previous architecture (v0.6.x) used a SIGALRM-based "canary": Python's NFA regex engine
...
plugin API. It also prevented users of other engines from accessing the full credential scanner and path traversal guard
Zenzic hardening that only the CLI pipeline provides. A Sovereign CLI decouples quality
