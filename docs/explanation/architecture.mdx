...
> See [CI/CD Integration](../how-to/configure-ci-cd.mdx) for workflow examples.
...
> See [CI/CD Integration](../how-to/configure-ci-cd.mdx) for step-by-step workflows corresponding to each scenario.
...
Zenzic addresses this through the [`[governance]`](../reference/configuration-reference.mdx) configuration block and the [Z601 BRAND_OBSOLESCENCE](../reference/finding-codes.mdx#z601) finding. Format-aware, per-line suppression is available for intentional historical references using the appropriate comment syntax for the file type (`.md` vs. `.mdx`).
