...
| `workers != 1` and file count >= 50 | Parallel | `ThreadPoolExecutor` with per-file distribution |
...
Running `just verify` in `zenzic-doc` executes the full lifecycle-gate flow with one entry-point. External contributors
