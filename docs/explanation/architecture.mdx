...
The `get_adapter()` factory queries the `zenzic.adapters` entry-point group and falls back to `StandaloneAdapter` if no adapter is found. Third-party adapters register via `pyproject.toml` — see the [Adapter implementation guide](/developers/how-to/implement-adapter).
