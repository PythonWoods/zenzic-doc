...
Every entry in the VSM, including those produced by an extra content root, carries a `Route.source` that resolves back to a real file on disk. The regression suite (`tests/test_docusaurus_blog_vsm.py::TestV07xReverseMapping`) asserts `(repo_root / route.source).is_file()` for every blog route, locking the contract that v0.7.x virtual routes (tags, pagination, authors) will inherit: a route with no physical origin would be a validator screaming `error` without ever saying `where`.

### Reverse-Mapping Invariant & Virtual Routes {#reverse-mapping}

Multi-Root Discovery (v0.7.x) solved **VSM Blindness** for physical files outside
`docs/`. v0.7.x extends the guarantee to **engine-generated pages**: Docusaurus renders
...
v0.7.x ships a first-class CLI command that exposes the VSM to external consumers:
...
Artificial Intelligence agents that require architectural context.
