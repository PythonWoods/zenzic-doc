...
> See [Getting Started](../how-to/install.mdx) for the `zenzic init` setup workflow.
...
> For field-level syntax and examples, see [Configuration Reference](../reference/configuration-reference.mdx).
...
> CLI syntax: see [CLI Reference — `inspect routes`](../reference/cli.mdx).
