...
Zenzic is designed to be an **Exclusion Zone** — a fixed, stable point that remains valid before,
...
- Third-party adapters install as Python packages and are discovered at runtime via entry-points.
  ...
  Adding support for a new engine (Hugo, Docusaurus, Sphinx) requires no Zenzic changelog entry.
...
documentation quality today, using Standalone Mode with zero configuration.
...
## Frictionless credential scanning
...
- **Standalone Mode for nav-agnostic projects.** When Zenzic has no nav declaration to compare
...
**Credential Scanner IO** — Every frontmatter line passes through `safe_read_line()`
before any parser sees it. If a credential pattern matches (OpenAI key,
GitHub token, AWS key, etc.), Zenzic raises `CredentialViolation` and aborts
with Exit 2 — the line is never stored or parsed. This makes the credential scanner an
...
| Site size | Pass 1 | Pass 1.5 | Pass 2 | Total |
| :--- | :--- | :--- | :--- | :--- |
...
4 GB RAM). The credential scanner scan (Pass 1, overlapping) adds < 10% overhead
...
Markdown example and commits it. The reference pipeline's **credential scanner** component addresses this
...
Exit code `2` is reserved exclusively for credential scanner events (credential detection). It can never be
suppressed by `--exit-zero`, `--strict`, or any other flag. A credential scanner detection is a build-blocking
security incident — by design.

Exit code `3` — the **path traversal guard** — guards against system-path traversal. When a Markdown
link references a path that escapes the documentation root (e.g. `../../etc/passwd`), Zenzic
terminates immediately with exit code 3. Like the credential scanner, the path traversal guard cannot be suppressed
or downgraded. These two non-negotiable exit codes form Zenzic's security boundary: no
...
Zenzic introduced the **Headless Architecture**: the verification engine (`zenzic`) and
...
The Headless Architecture is the structural implementation of the Exclusion Zone principle: the
...
| **Agnostic** | Standalone | any | `StandaloneAdapter` |
...
## The Design Philosophy {#philosophy}

Zenzic is built around three core properties: **transparent** (deterministic, reproducible,
source-only analysis), **hard** (non-suppressible security gates, strict mode by default in CI),
and **precise** (zero false positives as the design target, not the marketing claim).

These properties are not aspirational — they are constraints enforced at the architecture level.
Every design decision in Zenzic is made with this triad in mind: the tool must earn the right to
interrupt a developer's workflow by being unfailingly correct when it does.

---

*Zenzic is proudly built by PythonWoods. Although its heart beats in Python,
its mission is universal: to provide a Exclusion Zone for technical documentation,
