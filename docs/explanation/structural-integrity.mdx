---
sidebar_position: 9
sidebar_label: The Sentinel's Filter
title: The Sentinel vs. Proofreader
...
## The Sentinel's Filter {#the-sentinels-filter}

Every rule that ships in the Quartz Core must pass a three-dimensional admission test.
We call this **The Sentinel's Filter**: a rule enters Zenzic if and only if it defends
...
| :--- | :--- | :--- | :---: |
| [`Z201`](../reference/finding-codes.mdx#z201) | `SHIELD_SECRET` | Credentials, API keys, tokens in any source line | 2 |
...
See [Safe Harbor](./safe-harbor.mdx) and [The Zenzic Trinity](./the-zenzic-trinity.mdx)
...
| [`Z106`](../reference/finding-codes.mdx#z106) | `CIRCULAR_LINK` | Link that form a circular reference cycle |
...
Node ≥ 18 required     Python 3.11+ required
...
permanent. If a proposed rule does not pass The Sentinel's Filter, it does not ship.
...
These categories are not beneath Zenzic — they are **outside its mandate**. The Sentinel
...
- [Safe Harbor](./safe-harbor.mdx) — The exit code contract and the inviolable security gate
