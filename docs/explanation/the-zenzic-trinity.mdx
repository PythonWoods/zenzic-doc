...
[Exclusion Zone](./privacy-gate.mdx), Zenzic is organized into a Trinity of Integrity: three
...
| **Layered Exclusion Manager** | Enforces a four-level hierarchy (VCS ignores → system rules → user config → CLI flags) to guarantee a clean scan scope. |
