...
AST-level repository cartography is available through the deterministic rule surface
and exported via `zenzic inspect` commands for external context integration.
