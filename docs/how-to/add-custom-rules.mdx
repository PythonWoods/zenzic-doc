...
<div className="text-zinc-500 ml-6 text-[11px]">Remove DRAFT marker before publishing.</div>
...
the project uses `MkDocsAdapter`, `ZensicalAdapter`, or `StandaloneAdapter`. The rule engine
operates on raw Markdown text; it has no knowledge of the build engine. This means:

- Rules you write for a MkDocs project require no changes after [migrating to Zensical](./migrate-engines.mdx).
