...
**Type:** table — **Default:** `engine = "auto"`, `default_locale = "en"`, `locales = []`,
...
Selects the adapter used for nav extraction and i18n path resolution. When `"auto"` (the
default), Zenzic probes the project root for known engine config files in priority order:
`zensical.toml` → `docusaurus.config.ts` → `mkdocs.yml` → `StandaloneAdapter`. Valid values
for a stock Zenzic installation are `"auto"`, `"mkdocs"`, `"zensical"`, `"docusaurus"`, and
`"standalone"`. Third-party adapters registered under the `zenzic.adapters` entry-point group
...
| :--- | :--- | :--- |
| `"auto"` **(default)** | — | `discover_engine()` probes for manifests; see Auto-Discovery priority above |
| `"mkdocs"` | `mkdocs.yml` found | `MkDocsAdapter` — reads nav and i18n from `mkdocs.yml` |
| `"mkdocs"` | `mkdocs.yml` absent, no locales | `StandaloneAdapter` — no nav awareness |
...
| any unknown string | — | `StandaloneAdapter` — no registered adapter for this engine |

:::tip[Engine config files are automatically excluded]
Each adapter protects its engine configuration file from the unused-assets check (Z405). You do
**not** need to add `docusaurus.config.ts`, `mkdocs.yml`, or `zensical.toml` to `excluded_assets`
in `.zenzic.toml`. See [System Guardrails](../reference/configuration-reference.mdx#excluded-dirs) for the
full list of automatically excluded files.
:::

### Standalone mode

When `StandaloneAdapter` is selected, Zenzic has no knowledge of the project's nav structure. In
this mode:
...
Standalone mode is the correct behaviour for plain Markdown repositories, wikis, and projects that
...
Three nav entry forms are supported and freely mixable:
...
adapter reads this flag and adjusts `get_route_info()` routing accordingly.
