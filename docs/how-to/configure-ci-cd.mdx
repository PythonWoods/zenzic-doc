...
Exit 4 (quality regression via `zenzic diff`) respects `fail-on-error`.
...
The Zenzic Quality Gate uses `zenzic diff` to compare the current score against a saved baseline. If the score dropped, exit code 4 is emitted and the PR is blocked. This is the recommended setup for teams that want to enforce documentation quality as a hard merge gate.
...
3. **Verdict**: If the score dropped or suppression debt exceeded the cap, exit 4 is emitted — the PR check fails.
...
:::info[Exit 4 — Quality Regression]
When exit 4 fires, the PR annotation reads: *"Documentation quality score dropped vs baseline. The Zenzic Quality Gate blocked this PR."* Add `fail-on-error: "false"` to make exit 4 observational (warning) rather than blocking.
:::
...
`zenzic diff` compares the current score against the saved `.zenzic-score.json` baseline and exits with code 4 if the score dropped:

```yaml title=".github/workflows/zenzic.yml"
- name: Detect score regression
  run: |
    uvx zenzic score --save        # update snapshot
    uvx zenzic diff --threshold 5  # exit 4 if score drops > 5 points
```
...
| **`4`** | **Quality regression: score dropped vs baseline** | **Review `zenzic diff` output; block merge** |
