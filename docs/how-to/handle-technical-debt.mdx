...
The cost **escalates** once you exceed the cap: each suppression beyond 30 costs **2 pts** instead of 1.
...
The escalation is intentional: accumulating suppressions beyond the governance cap is a signal that the project has structural documentation debt that cannot be papered over.
...
A `<!-- zenzic:ignore Z201 -->` comment is **silently ignored**. The finding is still emitted. The exit code is still 2 or 3. The score collapses to 0.
