...
The repository ships maintained Z-code gallery fixtures that mirror the documented contracts:

- `examples/z101-broken-links/` — link integrity fixture (Z101 LINK_BROKEN, exit 1).
- `examples/z201-credentials/` — credential scanner fixture (Z201 CREDENTIAL_SECRET, exit 2).
- `examples/z405-unused-assets/` — unreferenced asset fixture (Z405, exit 1).
- `examples/z601-brand-obsolescence/` — brand obsolescence fixture (Z601, exit 1).
- `examples/z602-i18n-parity/` — i18n parity fixture (Z602, exit 1).

:::tip[Just want to run it now?]

```bash
uvx zenzic lab      # explore all 5 Z-code gallery scenarios — zero setup
uvx zenzic check all  # protect your own documentation
```

Curious how Zenzic handles credential detection or i18n parity gaps?
Launch `zenzic lab` and watch Zenzic in action across every Z-code gallery scenario.
:::

import { SummaryTerminal } from '@site/src/components/ZenzicTerminal';
...
- **Ghost Routes** — when `mkdocs-material` with `reconfigure_material: true` generates locale
  ...
  marks them `REACHABLE` automatically in the [Virtual Site Map](../reference/checks#vsm-how-it-works)
  so they are never reported as false orphans — with no manual exclusion list required.

### Standalone mode {#standalone-mode}

When no build-engine configuration is found, Zenzic falls back to `StandaloneAdapter`. In this mode:
...
Standalone mode is the right choice for plain Markdown wikis, GitHub-wiki repos, or any project
...
```bash
zenzic check all --engine standalone    # skip orphan check regardless of config files
