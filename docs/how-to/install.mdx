...
- `examples/zensical-basic/` — Zensical current release baseline (`[project].nav`).
- `examples/broken-docs/` — intentional failures (exit 1).
- `examples/security_lab/` — security fixture (credential scanner exit 2, path traversal guard exit 3).

:::tip[Just want to run it now?]

```bash
uvx zenzic lab      # explore all 9 built-in examples — zero setup
...
Launch `zenzic lab` and watch Zenzic in action across every supported engine.
:::
