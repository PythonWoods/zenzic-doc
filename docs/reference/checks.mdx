...
:::info[Topological telemetry — suppressed by default]

`CIRCULAR_LINK` findings are hidden from standard output because they are not violations. Use `--show-info` to surface them as structural metrics:

```bash
zenzic check all --show-info
```

They never affect exit codes in either normal or `--strict` mode. An unusually high cycle density on a single page may be worth reviewing as an Information Architecture signal — but that judgment belongs to the engineer, not to the analyzer.
:::
