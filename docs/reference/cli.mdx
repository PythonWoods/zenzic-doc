...
zenzic score --stamp           # Update <!-- zenzic:badge --> in badge_stamp_files
...
Updates the `<!-- zenzic:badge -->` marker in all files listed in `badge_stamp_files` (default: `README.md`).
The Shields.io badge URL on the line immediately following the marker is replaced with the current
score and the brand color:
...
See [Official Badges](/docs/how-to/add-badges) for setup and the full CI/CD snippet.
