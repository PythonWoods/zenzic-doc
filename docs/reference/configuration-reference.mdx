...
### Governance Suppression Contract (Phase 2)
...
When set to `"auto"` (the default), Zenzic probes the project root at runtime using **Quartz Auto-Discovery**, scanning for engine config files in priority order:
...
**Pattern matching:** case-sensitive whole-word scan. The term `"Shield"` does not match `"ShieldCheck"` or `"shields.io"`.
