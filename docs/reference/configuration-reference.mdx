...
> See [Exclusion Design](../explanation/exclusion-design.mdx) for the rationale behind conscious exclusion vs. blind VCS automation.
...
When `true`, Zenzic reads `.gitignore` files from the repository root and docs directory and excludes matching files from all checks. Enabled by default — see [Exclusion Design](../explanation/exclusion-design.mdx) for operational guidance.
...
### `cache_external_links` {#cache-external-links}
...
When `true`, external link validation results are atomically cached on disk (`.zenzic_cache/external_links.json`). Setting this to `false` disables the caching subsystem entirely.
...
> See [Exclusion Design — Governance Score Math](../explanation/exclusion-design.mdx#governance-score-math) for the flat-cost model and hybrid governance policy design.
