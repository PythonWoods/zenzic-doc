...
Files updated by `zenzic score --stamp`. Each file must contain a `<!-- zenzic:badge -->` HTML comment marker; the Shields.io badge URL on the line immediately following the marker is replaced with the current score URL and brand color (indigo `4f46e5` for 100, amber `f59e0b` for passing, red `ef4444` for fail or security override).
...
Add a `<!-- zenzic:badge -->` marker to each listed file, followed on the next line by any Shields.io badge as a placeholder. See [Official Badges](/docs/how-to/add-badges) for the complete setup guide.
