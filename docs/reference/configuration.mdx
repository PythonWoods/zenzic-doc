...
Zenzic automatically excludes certain files from all quality checks — including the unused-assets check (Z903). These **Level 1 Guardrails** are immutable and require no user configuration.
...
Running `zenzic check all .` from the project root will never produce Z903 warnings on your
