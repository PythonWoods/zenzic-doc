...
[CLI Commands: Global flags](./cli.mdx#global-flags) to tune policy per run:
...
writing a single line of `zensical.toml`. When the bridge activates, the Sentinel banner
notifies you:
...
[Download Guide](pathname:///assets/guide.pdf)
```
