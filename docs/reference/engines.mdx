...
### Blog auto-discovery {#docusaurus-blog}
...
- Assets referenced **only** from a blog post no longer trigger Z903 (Unused Asset).
...
:::note[Pagination and Authors — Phase 2]
Pagination routes (`/blog/page/{n}/`) and author routes will be added in subsequent
phases. The `VirtualRoute.kind` field already reserves the literals `"pagination"`,
