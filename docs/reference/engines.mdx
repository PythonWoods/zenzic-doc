...
Zenzic ships adapters for specific major-version lines. Declaring a different engine is a configuration error: Zenzic will emit `Z001 CONFIG_ERROR` and abort.
...
> See [Configuration Loading — Agnostic Citizen chain](../explanation/configuration-loading.mdx) for the architectural rationale behind Zensical's native i18n versus MkDocs plugin indirection.
