...
The credential scanner detected a known credential pattern. Speculative Base64 decoding is applied — encoded tokens that decode to credential patterns are also flagged.
...
A relative path uses `..` segments to escape the `docs/` root boundary, potentially exposing private repository files (`.env`, `config.py`).
...
A reference-style link (`[my link][ref]`) exists but its definition (`[ref]: http://...`) is missing. Most renderers silently degrade the link to plain text.
