...
Each suppression comment silences **only** the specified code on that line. To suppress multiple codes, add multiple comments. To exempt an entire file, use `obsolete_names_exclude_patterns` (Z601) or the relevant `excluded_files` patterns in `.zenzic.toml`.

:::tip[Use suppression sparingly]
Suppression is for intentional, documented decisions — not for silencing findings you haven't reviewed. Every suppressed line should have a corresponding comment explaining why.
:::

See [Suppression Policy](./suppression-policy.mdx) for the full reference, including `--strict` mode, the Suppression Debt model, and the `--audit` override.
