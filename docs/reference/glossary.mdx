...
A 0--100 quality score computed across four weighted categories: Structural (40%), Content (30%), Navigation (20%), and Brand & Assets (10%). Each category accumulates per-code penalty deductions (D092 — Zenzic Penalty Scorer). The score is computed by `zenzic score` and can be gated in CI via `fail_under` in `zenzic.toml` or `--fail-under` on the CLI.
