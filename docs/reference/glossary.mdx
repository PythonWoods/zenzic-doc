...
The design philosophy behind Zenzic's file discovery and exclusion model. Exclusion Zone guarantees three properties:
...
### Zenzic Score {#sentinel-score}

A 0--100 quality score computed across four weighted categories: Structural (40%), Content (30%), Navigation (20%), and Brand & Assets (10%). Each category accumulates per-code penalty deductions (D092 — Quartz Penalty Scorer). The score is computed by `zenzic score` and can be gated in CI via `fail_under` in `zenzic.toml` or `--fail-under` on the CLI.
...
Pass 2 only begins when Pass 1 completes without credential scanner findings. This is the credential scanner-as-firewall guarantee: no further analysis is performed on files that contain leaked credentials.
