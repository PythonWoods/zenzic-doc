...
Prior to v0.8.0, suppressions within `suppression_cap` were free (allowance model). Since v0.8.0, every suppression deducts 1 point (flat-cost model). `suppression_cap` is now exclusively a hard-fail exit threshold. Projects upgrading from v0.7.x will see their maximum achievable score reduced by their active suppression count.
:::
