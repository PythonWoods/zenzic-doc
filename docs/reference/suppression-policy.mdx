...
The map key is a glob pattern relative to `docs_dir`. Matched findings are **silently dropped before display** with **zero suppression debt cost** — they are never counted in the Suppression Audit footer.
