...
`zenzic lab` runs every scanner against pre-built fixtures: a real security breach and a real clean report — in 60 seconds.
:::
...
**Credential scanner demo** — run act 2 (leaked credential demo):
...
**Want to see the credential scanner in action?**
...
→ Pick any act from the menu, or run a specific one: `uvx zenzic lab 2` (credential scanner), `uvx zenzic lab 0` (The Seal).
:::
