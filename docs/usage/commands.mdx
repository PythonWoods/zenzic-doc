...
all adapters (MkDocs and Zensical) to resolve URLs **without** `use_directory_urls`, 
producing flat `.html` paths instead of clean directory-based slugs.
...
:::note[Engine parity]
The `--offline` flag has **identical behaviour** on both the MkDocs and Zensical adapters.
