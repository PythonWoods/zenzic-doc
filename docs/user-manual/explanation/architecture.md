...
This page describes the internal design of Zenzic for contributors and advanced users who need to understand how the tool works under the hood. For configuration and usage, see the [Configuration Reference](../reference/configuration-reference.md) and [Checks Reference](../reference/checks.md).
...
<div className="zz-figure">
  ![Zenzic Three-Phase Analysis Pipeline: File Set → Pass 1 (credential scanner + Content streams) → Pass 2 (Cross-Check) → Pass 3 (Integrity Report) → Quality Report](../../assets/brand/svg/zenzic-pipeline-flow.svg)
</div>
...
<div className="zz-figure zz-figure--centered">
  ![Zenzic credential scanner normalization algorithm: 8-step pipeline from Raw Line through ZRT-006/007/003 normalization passes to dual raw/normalized scan, deduplication, and SecurityFinding output](../../assets/brand/svg/zenzic-credential-normalization.svg)
</div>
...
<div className="zz-figure">
  ![Zenzic Virtual Site Map: projection from physical filesystem files through VSMBuilder and Adapter Protocol to a route catalog with REACHABLE, ORPHAN, and GHOST route statuses](../../assets/brand/svg/zenzic-vsm-projection.svg)
</div>
...
Third-party adapters register via `pyproject.toml` — see the [Adapter implementation guide](../../contributor-guide/how-to/implement-adapter.md).
...
> [CI/CD Integration guide](../how-to/configure-ci-cd.md#doc-code-parity).
