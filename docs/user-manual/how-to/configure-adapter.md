...
```toml
[build_context]
engine         = "auto"     # "auto" (default), "mkdocs", "zensical", "docusaurus"
...
2. `docusaurus.config.ts` / `docusaurus.config.js` → `DocusaurusAdapter`
...
See [Writing an Adapter](../../contributor-guide/how-to/implement-adapter.md) for the full protocol.
