...
| **Missing alt-text** — `![](../../reference/url)` or `<img>` with blank/absent alt | warning | No |
...
- **Markdown inline images** — `![](../../reference/url)` or `![   ](../../reference/url)` (blank alt string)
- **HTML `<img>` tags** — `<img src="../../reference/...">` with no `alt` attribute, or `alt=""` with no
...
See [Writing Plugin Rules](../../contributor-guide/how-to/write-plugin.md) for the complete contract,
...
```text
  ...
  — add it to nav in mkdocs.yml or remove the link
    │ - [Secret page](../../reference/guide/secret.md)
```
...
which exists on disk but is not listed in the site navigation (UNREACHABLE_LINK) —
add it to nav in mkdocs.yml or remove the link
  │ - [Private Notes](../../reference/_private/notes.md)
```
