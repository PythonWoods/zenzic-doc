...
- `[text](../../reference/missing-page.md)` → target file not found
- `[text](../../reference/page.md#missing-anchor)` → anchor not found in target
...
```terminal title="PATH TRAVERSAL GUARD — PATH TRAVERSAL"
...
An asset is considered **used** if it appears as a Markdown image link (`![alt](../../reference/path)`) or an HTML `<img src="../../reference/...">` tag in any `.md` file. Paths are normalised using POSIX path arithmetic.
