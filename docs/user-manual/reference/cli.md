...
```terminal title="CAP EXCEEDED"
✘ Active Suppressions: 43
Global Cap: 30
Action: Reduce suppressions below the cap or update the configuration.
Remediation: /developers/how-to/release-governance-protocol
```
...
[Checks: Path Traversal Guard](checks.md#path-traversal-guard) for details.
:::
...
```terminal title="zenzic check all"
✘ [Z104] FILE_NOT_FOUND — exit 1
```

**Exit 2 — credential scanner security breach**

```terminal title="zenzic check all"
✘ SECURITY BREACH — exit 2
```
...
see [API JSON Contract](api-json.md).
...
[Zenzic GitHub Action](../how-to/configure-ci-cd.md#github-actions-zenzic-credential-gate) —
...
See [Writing an Adapter](../../contributor-guide/how-to/implement-adapter.md).
