...
comando (`check`, `guard`, `score`, `diff`, `clean`) mantiene il default
rigoroso (`fallback_to_cwd=False`) e continuerà a fallire esplicitamente
fuori da un perimetro di progetto.

```python
# src/zenzic/cli/_standalone.py — l'unico call site autorizzato per fallback_to_cwd=True
def init(path=None, plugin=None, force=False):
...
def check_all(path=None, strict=False):
