---
sidebar_label: "ADR 020: Completezza Audit Parallelo"
sidebar_position: 10
...
Nell'implementazione precedente alla current release, il coordinatore raccoglieva i
...
il generatore indefinitamente. La protezione ZRT-002 (Z009 per i worker in
...
> **Dalla current release, il coordinatore parallelo usa `concurrent.futures.wait()` con
...
**Cambiamenti comportamentali nella current release:**

| Scenario | Pre-current release | current release |
|---|---|---|
...
| Worker in deadlock | Z009 dopo 30 s per-worker | Z009 se nessun worker completa in 30 s |
...
tutti i future pendenti vengono cancellati e un finding Z009 viene emesso per
...
`_worker()` e `_scan_single_file()` sono **invariati** nella current release A parità
...
attiva incondizionatamente il guard Z009 — senza meccanismi aggiuntivi.
...
- Deadlock guard ZRT-002: se nessun future completa entro `_WORKER_TIMEOUT_S`
  secondi, tutti i future rimanenti vengono cancellati e un finding Z009 viene
