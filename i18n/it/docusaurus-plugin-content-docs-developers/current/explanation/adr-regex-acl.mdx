...
**Decisore:** Tech Lead
**Data:** 2026-05-10 (current release)
...
Quando pattern legacy usavano costrutti validi solo per la stdlib, come
...
2. **Tutti gli import regex di produzione passano attraverso l'ACL.** I moduli
   core devono usare `from zenzic.core import regex as re` invece di importare
   `re2` direttamente.
...
- [ADR 002: Zero Subprocesses Policy](./adr-zero-subprocesses.mdx) — il layer
  regex deve rimanere in-process e deterministico.
- [ADR 011: Cross-Instance Absolute Path Allowlist](./adr-cross-instance-allowlist.mdx)
  — un altro esempio di boundary architetturale stretto che assorbe un mismatch
  engine-specific senza contaminare il contenuto sorgente.
