...
1. Registra l'identità engine nella configurazione del progetto tramite `[build_context] engine`
   ...
   (vedi [Adapter e Configurazione del Motore](/docs/how-to/configure-adapter)).

2. Valida il comportamento dell'adapter in policy Sentinel strict:

   `zenzic check all --engine myengine --strict`.
   Per i controlli di esecuzione, vedi [Comandi CLI: Flag globali](/docs/reference/cli#global-flags).
