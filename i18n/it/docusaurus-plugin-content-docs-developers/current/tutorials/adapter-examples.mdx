...
```python
def provides_index(self, directory_path: Path) -> bool:
    # Nessun config motore — solo un index.md minimale segnala una landing page.
...
Quando `get_nav_paths()` restituisce un frozenset vuoto, `get_route_info()` tratta
...
`_classify_route(rel, nav_paths)` è un helper privato che `get_route_info()` chiama internamente
per mappare il percorso di un file sorgente al suo stato di route.
...
def get_route_info(self, rel: Path) -> RouteMetadata:
    """Restituisce i metadati di route derivati puramente dal filesystem.

    StandaloneAdapter non ha config motore, nav, né supporto slug.
    Ogni file è ``REACHABLE`` con un URL derivato dal filesystem.
    """
...
in `_classify_route()`, che `get_route_info()` chiama internamente.
`StandaloneAdapter` non ha un helper `_classify_route()` — `get_route_info()` restituisce
`REACHABLE` direttamente perché senza motore non esiste un contratto di navigazione.
