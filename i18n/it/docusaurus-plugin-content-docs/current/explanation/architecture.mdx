...
<div className="zz-figure">
  <ThemedImage
    alt="Zenzic — Pipeline a Tre Fasi: File Set → Passata 1 (credential scanner + Content stream) → Passata 2 (Cross-Check) → Passata 3 (Integrity Report) → Quality Report"
    sources={{
...
- **Core Maintainer**: `ZENZIC_CORE_PATH` impostato → `uv run --project <percorso>` sul sorgente locale.
