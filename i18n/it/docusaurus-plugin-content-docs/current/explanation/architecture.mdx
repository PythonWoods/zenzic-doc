...
<div style={{borderRadius:'8px',overflow:'hidden',margin:'1.5rem 0'}}>
  <ThemedImage
    ...
    sources={{
      light: useBaseUrl('/assets/brand/svg/zenzic-shield-normalization-light.svg'),
      dark: useBaseUrl('/assets/brand/svg/zenzic-shield-normalization.svg'),
    }}
...
A partire dalla v0.6.1 "Quartz Clarity", la VSM è **consapevole delle versioni**. Per gli adapter che supportano la documentazione multi-versione (attualmente `DocusaurusAdapter`), il VSM builder:
...
vettori d'attacco red-team identici producono findings identici su engine `standalone`,
`mkdocs` e `zensical`. I fixture blue-team producono un Zenzic Audit Badge identico su tutti
...
flowchart LR
    ...
    HAS_CONFIG -->|No| VANILLA["Fallback: StandaloneAdapter()"]
