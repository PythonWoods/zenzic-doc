...
<div style={{borderRadius:'8px',overflow:'hidden',margin:'1.5rem 0'}}>
  <ThemedImage
    alt="Zenzic — Pipeline a Due Passate: File Set → Passata 1 (Shield + Content stream) → Passata 2 (Cross-Check) → Passata 3 (Integrity Report) → Quality Report"
    sources={{
...
<div style={{borderRadius:'8px',overflow:'hidden',margin:'1.5rem 0'}}>
  <ThemedImage
    alt="Zenzic Shield — algoritmo di normalizzazione in 8 passaggi: dalla riga grezza attraverso ZRT-006/007/003 fino alla scansione duale e al SecurityFinding"
    sources={{
...
`mkdocs` e `zensical`. I fixture blue-team producono un Sentinel Seal identico su tutti
...
Passo 3 — Guardia Sandbox CEO-043
          ...
          come nuovo repo_root in modo che Blood Sentinel protegga le fughe dal
...
| Guardia sandbox | Il perimetro di Blood Sentinel è ancorato a `docs_root`, non a `cwd` |
...
all'hardening completo di Shield e Blood Sentinel, disponibile solo nella pipeline CLI.
...
Questo produce un'applicazione identica con VSM completo, Shield (ZRT-006/007) e Blood Sentinel
...
| `get_ui()` | Il singleton `SentinelUI` corrente (avvolge la console) |
...
| Aggiungere una utility condivisa | Aggiungere in `_shared.py` e importare via `from . import _shared` — non istanziare mai `Console` o `SentinelUI` localmente |
