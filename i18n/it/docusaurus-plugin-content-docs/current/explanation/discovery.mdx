...
> Vedi [Per iniziare](../how-to/install.mdx) per il workflow di setup con `zenzic init`.
...
> Per la sintassi dei campi e gli esempi, vedi [Riferimento alla Configurazione](../reference/configuration-reference.mdx).
...
> Sintassi CLI: vedi [Riferimento CLI — `inspect routes`](../reference/cli.mdx).
