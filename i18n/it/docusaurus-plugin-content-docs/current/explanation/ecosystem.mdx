...
Zenzic current release non fornisce plugin per motori di build né integrazioni interne.
...
Il modello Exclusion Zone va oltre la correttezza strutturale. Una base di codice o una suite di documentazione che contiene identificatori di brand obsoleti (codename di release non più in uso) porta un tipo di debito diverso: **debito narrativo**. Una pagina current release che fa ancora riferimento a un codename current series contraddice la cronologia delle versioni che cerca di documentare.
