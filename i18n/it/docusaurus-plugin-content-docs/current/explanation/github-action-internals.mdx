...
| `4` | **QUALITÀ** — regressione del punteggio rispetto alla baseline (`zenzic diff`) | ❌ Mai |
...
- name: Run Zenzic Documentation Quality Gate
  ...
  with:
    version: "0.7.1"
    guard-scan: "true"   # esegue zenzic guard scan prima di check all
    format: sarif
    upload-sarif: "true"
```
...
| `4` | **❌ Regressione Qualità** | Punteggio, debito di soppressione, link Playbook |
...
| [ADR Vault](https://zenzic.dev/it/developers/explanation/adr-vault) | Decisioni architetturali dietro il contratto exit code e il path traversal guard |
