...
|-----------|---------|----------------|---------|
| **Integrità Strutturale** | `zenzic check links [PATH]` | [Z101], [Z102], [Z103], [Z104], [Z105], [Z106], [Z107], [Z108] | **40 %** |
| **Eccellenza dei Contenuti** | `zenzic check all [PATH]` | [Z501], [Z502], [Z503], [Z505] | **30 %** |
| **Navigazione & SEO** | `zenzic check orphans [PATH]` | [Z402] | **20 %** |
| **Brand & Asset** | `zenzic check assets [PATH]` | [Z405], [Z406], [Z601] | **10 %** |
...
[Z106]: ../reference/finding-codes.mdx#z106
...
chiamano a volte “Nav Isolation” è formalmente **Integrità del Nav Contract ([Z406])**.

Z406 si attiva quando un file dichiarato nella configurazione di navigazione del motore
(es. una voce `nav:` di `mkdocs.yml` o una voce esplicita di `sidebars.ts` in
Docusaurus) non esiste su disco. Ogni violazione Z406 contribuisce alla categoria **Brand & Asset** (10%) usando
la stessa penaltà per codice degli altri finding valutati (Z406: 2,0 pts per violazione).
...
- Non esistono asset inutilizzati (zero Z405)
- Non esistono violazioni del nav contract (zero Z406)
- Non esistono riferimenti di brand obsoleti (zero Z601)
- Il credential scanner non ha trovato credenziali in nessun file (zero Z201 — implicito, fa crollare il punteggio a 0)
