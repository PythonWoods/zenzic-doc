...
|----------|----------|-----------------|----------------------|
| **v0.6.x** | **Obsidian** {/* zenzic:ignore Z601 */} | Vetro vulcanico — formato sotto pressione estrema, bordo eccezionalmente tagliente | Credential scanner (Z2xx), path traversal guard, primo output SARIF, modello Exclusion Zone |
| **v0.7.x** | **Quartz** {/* zenzic-ignore: Z601 - historical release codename */} | Piezoelettrico — preciso, auto-oscillante, riferimento per la frequenza | Codici di finding (`Zxxx`), contratto dei codici di uscita, Virtual Site Map, compatibilità SARIF |
| **v0.8.x** | **Basalt** {/* zenzic-ignore: Z601 - historical release codename */} | Roccia vulcanica densa — rinforzo strutturale ad alta resistenza | Plugin SDK, stabilizzazione dell'adapter protocol, performance su larga scala |
...
- Intestazioni di sezione in `CHANGELOG.md` (es. `## [0.8.0] — Basalt`) {/* zenzic-ignore: Z601 - historical release codename */}
...
- Testo dei tutorial o delle guide how-to (usare il numero di versione)
