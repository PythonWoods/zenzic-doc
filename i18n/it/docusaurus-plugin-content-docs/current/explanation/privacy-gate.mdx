...
Zenzic è progettato per essere un **Exclusion Zone** — un punto fisso e stabile che rimane valido
...
- Gli adapter di terze parti si installano come pacchetti Python e vengono scoperti a runtime
  ...
  richiede alcuna voce di changelog di Zenzic.
...
comunque validare la qualità della propria documentazione oggi, usando la Standalone Mode
...
## Credential scanning senza attriti
...
- **Standalone Mode per progetti senza nav.** Quando Zenzic non ha dichiarazioni nav contro cui
...
flowchart LR
    ...
    S["📄 Sorgente\nfile .md / .mdx\n.zenzic.toml"]:::source
    CRED["🛡️ Credential Scanner IO\nsafe_read_line()\nCredentialViolation → Exit 2"]:::credentials
...
**Credential Scanner IO** — Ogni riga del frontmatter passa attraverso `safe_read_line()`
...
`CredentialViolation` e interrompe con Exit 2 — la riga non viene mai salvata né
parsata. Questo rende il credential scanner un middleware IO: si posiziona tra le letture
...
(2 vCPU, 4 GB RAM). La scansione credential scanner (Fase 1, sovrapposta) aggiunge < 10%
...
una chiave API in un esempio Markdown e la committa. Il componente **credential scanner** della pipeline di
...
Il codice di uscita `2` è riservato esclusivamente agli eventi credential scanner (rilevamento credenziali).
Non può mai essere soppresso da `--exit-zero`, `--strict` o qualsiasi altro flag. Un rilevamento
Il credential scanner blocca il build su un incidente di sicurezza per il build — by design.

Il codice di uscita `3` — il **path traversal guard** — protegge contro l'attraversamento di percorsi
di sistema. Quando un link Markdown referenzia un percorso che esce dalla root della documentazione
(es. `../../etc/passwd`), Zenzic termina immediatamente con codice di uscita 3. Come il credential scanner, il
il path traversal guard non può essere soppresso o declassato. Questi due codici di uscita non negoziabili
...
Zenzic ha introdotto l’**Architettura Headless**: il motore di verifica (`zenzic`) e il
...
L'Architettura Headless è l'implementazione strutturale del principio Exclusion Zone: il livello di
...
| **Agnostico** | Standalone | qualsiasi | `StandaloneAdapter` |
...
## La Filosofia di Design {#philosophy}

Zenzic è costruito su tre proprietà fondamentali: **trasparente** (analisi deterministica,
riproducibile, solo-sorgente), **solido** (gate di sicurezza non sopprimibili, modalità strict
di default in CI) e **preciso** (zero falsi positivi come obiettivo di design, non come claim
di marketing).

Queste proprietà non sono aspirazionali — sono vincoli applicati a livello architetturale.
Ogni decisione di design in Zenzic è presa con questa triade in mente: lo strumento deve
guadagnarsi il diritto di interrompere il flusso di lavoro di uno sviluppatore essendo
infallibilmente corretto quando lo fa.

---

*Zenzic è un progetto orgogliosamente sviluppato da PythonWoods. Sebbene il suo
cuore batta in Python, la sua missione è universale: fornire un Exclusion Zone per la
