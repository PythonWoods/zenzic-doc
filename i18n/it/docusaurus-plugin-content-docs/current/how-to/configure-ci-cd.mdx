...
Il Zenzic Quality Gate usa `zenzic diff` per confrontare il punteggio corrente con un baseline salvato. Se il punteggio è sceso, il check della PR fallisce. Questa è la configurazione raccomandata per i team che vogliono applicare la qualità della documentazione come gate di merge rigido.
...
3. **Verdetto**: Se il punteggio è sceso o il suppression debt ha superato il cap, il check della PR fallisce.
...
Quando viene rilevata una regressione del punteggio, l'annotazione PR recita: *"Il punteggio di qualità della documentazione è sceso rispetto al baseline. Il Zenzic Quality Gate ha bloccato questa PR."* Aggiungi `fail-on-error: "false"` per rendere il rilevamento di regressione osservazionale (warning) invece che bloccante.
:::
