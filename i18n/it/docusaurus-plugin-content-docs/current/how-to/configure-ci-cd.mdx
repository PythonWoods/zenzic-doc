...
`fail-on-error: "false"` sopprime solo exit 1 (finding di qualità) e exit 4 (regressione di qualità via `zenzic diff`). Exit 2 (Credential scanner — credenziale
...
Il Zenzic Quality Gate usa `zenzic diff` per confrontare il punteggio corrente con un baseline salvato. Se il punteggio è sceso, viene emesso l'exit code 4 e la PR viene bloccata. Questa è la configurazione raccomandata per i team che vogliono applicare la qualità della documentazione come gate di merge rigido.
...
3. **Verdetto**: Se il punteggio è sceso o il suppression debt ha superato il cap, viene emesso exit 4 — il check della PR fallisce.
...
:::info[Exit 4 — Regressione di Qualità]
Quando exit 4 scatta, l'annotazione PR recita: *"Il punteggio di qualità della documentazione è sceso rispetto al baseline. Il Zenzic Quality Gate ha bloccato questa PR."* Aggiungi `fail-on-error: "false"` per rendere exit 4 osservazionale (warning) invece che bloccante.
:::
...
steps:
  ...
  - name: 🔄 Aggiorna Gist per Badge
    ...
    uses: Schneegans/dynamic-badges-action@current release
    with:
...
`zenzic diff` confronta il punteggio corrente con il baseline `.zenzic-score.json` salvato e esce con il codice 4 se il punteggio è sceso:

```yaml title=".github/workflows/zenzic.yml"
- name: Rileva regressione del punteggio
  run: |
    uvx zenzic score --save        # aggiorna snapshot
    uvx zenzic diff --threshold 5  # exit 4 se il punteggio scende > 5 punti
```
...
| **`4`** | **Regressione di qualità: punteggio sceso rispetto al baseline** | **Esamina l'output di `zenzic diff`; blocca il merge** |
