...
## GitHub Actions: Gate Zenzic Shield {#github-actions-zenzic-shield-gate}
...
`fail-on-error: "false"` sopprime solo exit 1 (finding di qualità). Exit 2 (Shield — credenziale
rilevata) e exit 3 (Blood Sentinel — path traversal) **non sono mai sopprimibili** con alcun input.
...
## Recupero dallo Shield — Quando Viene Rilevata una Credenziale {#shield-recovery}
...
Il Sentinel Report ti dice tutto ciò di cui hai bisogno:
...
### Step 5 — Verifica che lo Shield sia soddisfatto

```bash
uvx zenzic check all
# Atteso: exit 0, sezione Shield mostra ✔ nessuna credenziale rilevata
```

Solo exit 0 dal pass Shield significa che il recupero è completo.
...
senza essere rilevato. Lo Shield di Zenzic previene questo intercettando le credenziali **prima**
che entrino nella pipeline CI — idealmente tramite un hook pre-commit. Vedi
[Integrazione pre-commit](#pre-commit) per aggiungere lo Shield come gate locale che
...
Lo Shield in CI è la tua ultima linea di difesa. L'hook pre-commit è la prima:
