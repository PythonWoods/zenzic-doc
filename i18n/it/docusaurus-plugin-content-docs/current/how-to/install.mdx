...
Il repository include fixture Z-code mantenuti e allineati ai contratti documentati:

- `examples/z101-broken-links/` — fixture integrità link (Z101 LINK_BROKEN, exit 1).
- `examples/z201-credentials/` — fixture credential scanner (Z201 CREDENTIAL_SECRET, exit 2).
- `examples/z405-unused-assets/` — fixture asset non referenziato (Z405, exit 1).
- `examples/z601-brand-obsolescence/` — fixture obsolescenza brand (Z601, exit 1).
- `examples/z602-i18n-parity/` — fixture parità i18n (Z602, exit 1).

:::tip[Vuoi eseguirlo subito?]

```bash
uvx zenzic lab      # esplora tutti i 5 scenari Z-code — zero setup
uvx zenzic check all  # proteggi la tua documentazione
```

Vuoi vedere come Zenzic rileva credenziali o gap di parità i18n?
Lancia `zenzic lab` e guarda Zenzic in azione su ogni scenario Z-code della galleria.
:::
...
- **Ghost Route** — quando `mkdocs-material` con `reconfigure_material: true` genera entry point
  locale (es. `/it/`) al momento della build, quelle pagine non appaiono mai in `nav:`. Zenzic
  le contrassegna come `REACHABLE` automaticamente nella [Virtual Site Map](../reference/checks#vsm-how-it-works)
  senza richiedere alcuna lista di esclusione manuale.
...
La modalità Standalone è la scelta giusta per wiki Markdown semplici, repository GitHub-wiki o
...
```bash
zenzic check all --engine standalone    # salta il controllo orfani
...
- [Funzionalità avanzate](../reference/advanced-features.mdx) — integrità dei riferimenti, credential scanner, utilizzo programmatico
