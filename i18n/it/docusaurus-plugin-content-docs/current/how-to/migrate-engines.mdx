...
La conseguenza pratica è che `MkDocsAdapter` preserva il contratto strutturale MkDocs 1.x come
una specifica statica con versione. Finché il tuo `mkdocs.yml` descrive una struttura MkDocs 1.x
valida, Zenzic la comprende e la valida — indipendentemente da ciò che qualsiasi binario di
build supporti o meno. Eseguendo `zenzic check all` con `engine = "mkdocs"` si testa il
contenuto rispetto a quel contratto documentato, non rispetto a nessuna versione binaria specifica.
