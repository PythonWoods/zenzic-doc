...
- Le soppressioni inline (`zenzic-ignore` / `zenzic:ignore`) vengono ignorate.
...
### Blueprint Governance-Ready (v0.8.0)

`zenzic init` ora genera una coppia di configurazioni Governance-Ready:

- `zenzic.toml` (costituzione condivisa, versionata)
- `.zenzic.local.toml` (sacrario locale macchina, ignorato da git)

Il file globale non è più un guscio vuoto: è un blueprint Basalt con commenti
...
Dovresti vedere immediatamente il tuo primo Sentinel Seal.
...
**Uscita 0 — Sentinel Seal**

<SentinelOutput status="success" exitCode={0} compact />

**Uscita 1 — Anomalie di qualità**

<SentinelOutput status="error" code="Z104" exitCode={1} compact />

**Uscita 2 — Violazione di sicurezza Shield**

<SentinelOutput status="breach" exitCode={2} compact />
...
| Z901–Z906 Sistema | `Z901`–`Z906` | `error` / `warning` |
...
Ogni atto dichiara il proprio esito atteso. Dopo l'esecuzione, il Sentinel Seal riporta
