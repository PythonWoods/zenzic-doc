...
Zenzic sui segnali non bloccanti, come la topologia dei cicli link (`CIRCULAR_LINK`).
...
la validazione HTTP HEAD concorrente degli URL esterni — lasciando il Passaggio 1 (credential scanner, Z201)
...
zenzic check links --no-external          # solo link interni; credential scanner (Z201) sempre attivo
```
...
| :--- | :--- |
| Passaggio 1 — credential scanner / rilevamento credenziali (Z201) | ❌ Mai saltato |
| Passaggio 1 — path traversal guard / path traversal (Z202/Z203) | ❌ Mai saltato |
...
**Sezione A — Scanner Core (Built-in):** sette scanner compilati in Zenzic. Il credential scanner (Z201)
...
<ZenzicOutput
  ...
  exitCode={0}
  scanners={[
    { codes: 'Z201',     scanner: 'Credential Scanner', capability: 'Rilevamento credenziali e sicurezza',          exit: '2', icon: '🛡', security: true },
    { codes: 'Z202–203', scanner: 'Path Traversal Guard', capability: 'Prevenzione path-traversal e jailbreak',       exit: '3', icon: '🔒', security: true },
    ...
    { codes: 'Z501–504', scanner: 'Content scanner', capability: 'Segnaposto, snippet e punteggio qualità',     exit: '1', icon: '📄' },
  ]}
/>
...
pacchetto contro Zenzic e riporta se ogni atto ha soddisfatto l'esito atteso.
...
| :--- | :---: | :---: | :--- |
| Guardrail OS & Ambiente | 🛡 | 0–3 | Linting base, credential scanner, run pulito |
...
Ogni atto dichiara il proprio esito atteso. Dopo l'esecuzione, il Zenzic Audit Badge riporta
