...
Le inclusioni forzate non possono mai sovrastare i System Guardrails (`.git`, `.venv`, ecc.). La regola e semplice: i System Guardrails sono immutabili.
...
Quando impostato su `"auto"` (il default), Zenzic analizza la root del progetto a runtime con la **Quartz Auto-Discovery**, cercando file di configurazione dell'engine nell'ordine di priorità:
