...
:::info Invariante Matematico — Modello Flat-Cost
Ogni soppressione attiva deduce esattamente **1 punto DQS** (modello flat-cost). Il punteggio massimo raggiungibile dal progetto è quindi:

$$\text{Punteggio Massimo Raggiungibile} = 100 - |F_s|$$

dove $|F_s|$ è il conteggio totale delle soppressioni attive. Configurare `fail_under > 100 - suppression_cap` crea una **contraddizione matematica**: il gate di punteggio si attiverà per il debito da soppressioni *prima* che venga raggiunto il cap, rendendo irraggiungibili gli slot superiori del budget del cap.

**Regola di configurazione sicura:** `fail_under` $\leq$ `100 - suppression_cap`.
:::

:::tip[Politiche di Governance Ibride]
Poiché `fail_under` e `suppression_cap` agiscono come **vincoli ortogonali**, è possibile progettare politiche di governance ibride per ambienti enterprise. Ad esempio, configurare `fail_under = 90` e `suppression_cap = 30` significa: "La qualità globale del repository (finding + debito) non deve mai scendere sotto 90/100, **ma** indipendentemente dal punteggio, rifiutiamo categoricamente più di 30 difetti soppressi." Questo impedisce ai team di nascondere debito strutturale massivo anche quando il codice attivo è altrimenti perfetto.
:::
