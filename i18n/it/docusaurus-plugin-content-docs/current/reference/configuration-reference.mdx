...
> Vedi [Design Esclusioni](../explanation/exclusion-design.mdx) per il razionale del controllo consapevole vs. automazione cieca.
...
Abilitato per default — vedi [Design Esclusioni](../explanation/exclusion-design.mdx) per la guida operativa. Le inclusioni forzate (`included_dirs`, `included_file_patterns`) hanno la precedenza sulle esclusioni VCS, ma i System Guardrails restano sempre applicati.
...
| Campo | Tipo | Predefinito | Descrizione |
| :--- | :--- | :--- | :--- |
| `cache_external_links` | `bool` | `true` | Se `true`, i risultati della validazione di rete vengono messi in cache localmente |

Quando `true`, i risultati della validazione dei link esterni vengono messi in cache atomica su disco (`.zenzic_cache/external_links.json`). Impostare questo valore a `false` disabilita completamente il sottosistema di caching.
...
> Vedi [Design Esclusioni](../explanation/exclusion-design.mdx) per il modello flat-cost e il design delle policy di governance ibride.
