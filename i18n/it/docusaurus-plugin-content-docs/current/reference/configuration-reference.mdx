...
`respect_vcs_ignore` è `true` per default. Questo allinea Zenzic al comportamento dei linter moderni (i percorsi ignorati da VCS vengono esclusi salvo inclusioni esplicite).
...
Abilitato per default — vedi [La Via Zenzic](#zenzic-way) qui sopra per la guida operativa. Le inclusioni forzate (`included_dirs`, `included_file_patterns`) hanno la precedenza sulle esclusioni VCS, ma i System Guardrails restano sempre applicati.
