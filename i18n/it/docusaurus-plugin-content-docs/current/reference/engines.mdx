...
> Vedi [Caricamento Configurazione](../explanation/configuration-loading.mdx) per il razionale architetturale dietro i18n nativo di Zensical vs. indirezione di plugin MkDocs.
...
configurazione di Docusaurus come **testo statico** usando pattern matching — nessun
...
### Partials e la Frontiera Sicurezza vs. Routing {#docusaurus-partials}

Docusaurus tratta qualsiasi file o directory il cui nome inizia con `_` come un **partial** — un frammento di contenuto privato che il motore di build esclude intenzionalmente dal routing. Esempi comuni includono i descrittori sidebar `_category_.json`, le directory `_partials/` con snippet MDX riutilizzabili, e qualsiasi file `_*.mdx` pensato per essere importato da altre pagine.

Zenzic applica una separazione rigorosa tra due layer:

| Layer | Comportamento | Motivazione |
| :--- | :--- | :--- |
| **I/O Discovery** (`walk_files`) | Legge **tutti** i file, inclusi quelli con prefisso `_` | Z201 HARDCODED_SECRET e Z204 FORBIDDEN_TERM devono scansionare ogni file — le credenziali nei partial sono ugualmente pericolose |
| **Routing** (`DocusaurusAdapter`) | Restituisce `status="IGNORED"` e `canonical_url=""` per qualsiasi segmento di percorso che inizia con `_` | I partial non hanno URL pubblico; emettere un URL fantasma causerebbe falsi positivi Z405 |
| **Regole Z402 / Z502** | Saltano i file con stato `IGNORED` | I controlli word-count e placeholder sono privi di significato per file senza audience |

:::caution[Sicurezza non negoziabile]
Il layer I/O **non filtra mai** i file con prefisso `_`. Farlo renderebbe il credential scanner cieco ai segreti nascosti nei file partial — un blindspot di sicurezza vietato dall'ADR-013. L'esclusione è puramente logica e avviene al layer di routing, **dopo** che la scansione di sicurezza è già avvenuta.
:::

Questo significa che non vedrai mai falsi positivi Z402/Z502/Z405 per le tue directory `_partials/`, mentre Z201/Z204 rileva comunque qualsiasi credenziale accidentalmente inserita lì.

### Supporto Monorepo — Site Root Dinamica {#docusaurus-monorepo}

I progetti Docusaurus sono spesso annidati all'interno di un monorepo più grande: la sorgente della documentazione risiede sotto `website/docs/` mentre `docusaurus.config.ts` si trova in `website/`. Zenzic gestisce questo automaticamente tramite la **discovery dinamica della site root**:

Quando `docs_dir = "website"` (o qualsiasi percorso che non è la root del repo), Zenzic risale da `docs_root` verso `repo_root` cercando `docusaurus.config.ts` o `docusaurus.config.js`. La prima directory antenata che contiene uno dei due file viene usata come site root per tutta l'analisi statica — `baseUrl`, `routeBasePath`, configurazione blog e percorsi sidebar vengono tutti estratti relativamente a quell'ancora.

```toml
# .zenzic.toml per un layout monorepo
# Root del repo: /project/
# docs_dir:      /project/website/
# Site root:     /project/website/  (dove risiede docusaurus.config.ts)
docs_dir = "website"
...
Nessuna configurazione aggiuntiva è necessaria. Se `docusaurus.config.ts` è assente da tutti gli antenati, Zenzic si ricade alla root del repo e emette un warning — non si blocca mai.

:::tip[Monorepo con versioning]
Per i monorepo con `versioned_docs/` contenente molti snapshot storici, aggiungilo a `excluded_dirs` per evitare l'amplificazione ×N dei finding. Gli snapshot storici sono immutabili — i loro finding non sono azionabili. Usa `[governance.directory_policies]` se hai bisogno di una audit trail a zero-debt invece dell'esclusione totale.
:::

### Auto-discovery del blog {#docusaurus-blog}
...
### Virtual Routes (Tag, Paginazione, Autori) {#docusaurus-virtual-routes}

Docusaurus genera rotte prive di file Markdown sorgente fisico: ogni tag univoco
nel frontmatter produce una pagina `/blog/tags/{slug}/`, gli indici paginati producono
pagine `/blog/page/{n}/`, e i profili autore producono pagine `/blog/authors/{id}/`.

`DocusaurusAdapter` inferisce queste virtual routes in modo statico — **nessun build step,
nessuna esecuzione Node.js** — leggendo i metadati frontmatter dai post del blog già
caricati in memoria.
...
Il set `source_files` è l'implementazione dell'**Invariante di Reverse-Mapping**: ogni
URL ammessa nella VSM — fisica o virtuale — deve risalire in modo non ambiguo a uno o
più file sorgente reali. Una `VirtualRoute` con `source_files` vuoto solleva `ValueError`
al momento della costruzione; non può raggiungere la VSM.
...
La tag index (`/blog/tags/`) elenca sempre l'**unione** di tutti i file blog che portano
almeno un tag, fornendo la tracciabilità cross-file necessaria per la diagnostica.

:::note[Paginazione e Autori — Fase 2]
Le route di paginazione (`/blog/page/{n}/`) e le route autore saranno aggiunte nelle
fasi successive. Il campo `VirtualRoute.kind` riserva già i letterali `"pagination"`,
`"author"` e `"author_index"` così i consumer downstream possono gestire tutti i tipi
di rotta senza un cambiamento di schema breaking.
:::

:::tip[Connessione a tool esterni]
Il comando `zenzic inspect routes` è ora disponibile. Questa funzionalità esporta la
mappa completa del sito in formato JSON deterministico. È progettata per essere
consumata da tool esterni: script Bash custom, dashboard di CI/CD, o tool
specializzati che necessitano di contesto architetturale.
:::
...
### Navigazione Unificata Discovery {#docusaurus-unified-nav}

**In Docusaurus, tutti i file in `docs/` sono instradati dal plugin docs** — il routing
è guidato dal file system, non dalla sidebar. Sidebar, navbar e footer controllano solo
la *UX di scoperta*: se un utente riesce effettivamente a trovare e cliccare una pagina.

Zenzic applica la **Legge di Scoperta UX**: un file è considerato `REACHABLE` se appare
in qualsiasi superficie di navigazione dell'interfaccia. Un file assente da tutte le
superfici è un orfano — ha un URL, ma nessun percorso utente conduce a esso.
...
:::note[Sidebar autogenerata]
Quando `sidebars.ts` contiene `type: 'autogenerated'`, Docusaurus mostra tutti i documenti
nella sidebar automaticamente. Zenzic lo rileva e marca **tutti i file** come `REACHABLE` —
nessun report orfano viene emesso indipendentemente dal contenuto di navbar o footer.
:::

I path URL `to:` vengono risolti rimuovendo i prefissi `baseUrl` e `routeBasePath`,
poi cercando su disco `.md` / `.mdx`. I link non-doc (es. `/blog/`, URL esterni)
non corrispondono mai a un file e vengono ignorati silenziosamente — nessun falso positivo.
