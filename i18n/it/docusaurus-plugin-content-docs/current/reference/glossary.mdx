...
La filosofia progettuale alla base del modello di file discovery ed esclusione di Zenzic. L'Exclusion Zone garantisce tre proprietà fondamentali:

1. **Determinismo** — Data la stessa configurazione e lo stesso stato del filesystem, Zenzic produce esattamente gli stessi file nello stesso ordine.
2. **Sicurezza per default** — I System Guardrails impediscono la scansione di VCS internals, ambienti virtuali e cache di build.
3. **Override esplicito** — Ogni inclusione ed esclusione è tracciabile a una specifica riga di configurazione o flag CLI.

La Privacy Gate (Exclusion Zone) definisce un confine preciso dove gli scanner di Zenzic sono intenzionalmente inibiti per proteggere metadati sensibili o pattern di sicurezza intenzionali.
...
La Passata 2 inizia solo quando la Passata 1 si completa senza finding del credential scanner. Questa è la garanzia "credential scanner come firewall": nessuna ulteriore analisi viene eseguita su file che contengono credenziali trapelate.

Vedi: [Architettura -- Two-Pass Pipeline](../explanation/architecture#two-pass-pipeline)
