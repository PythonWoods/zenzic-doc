...
La deduzione è limitata al massimo del tier Governance (25 pts) per evitare overflow. L'effetto pratico: un repository con 20 violazioni Z601 (10 in eccesso → moltiplicatore $= 2^2 = 4$) subisce quattro volte il normale impatto sulla governance.
...
dove $|F_s|$ è il conteggio totale delle soppressioni attive. Configurare `fail_under > 100 - suppression_cap` crea una contraddizione matematica: il gate di punteggio si attiva per il debito da soppressioni *prima* che venga raggiunto il cap di governance. Regola di configurazione sicura:

$$\texttt{fail\_under} \leq 100 - \texttt{suppression\_cap}$$
:::

:::tip[Architettura Dual-Gate — Vincoli Ortogonali]
...
- **Score gate:** `score < fail_under` → `zenzic score` termina con codice 1
- **Governance cap:** `|F_s| > suppression_cap` → `zenzic score` termina con codice 1
...
- L'impatto sul punteggio si applica solo alle soppressioni oltre `suppression_cap`.
...
:::warning Breaking Change — v0.8.0 (ADR-031)
Prima della v0.8.0, le soppressioni entro `suppression_cap` erano gratuite (modello allowance). Dalla v0.8.0, ogni soppressione deduce 1 punto (modello flat-cost). `suppression_cap` è ora esclusivamente una soglia di hard-fail per l'exit code. I progetti che aggiornano dalla v0.7.x vedranno il loro punteggio massimo raggiungibile ridotto del numero di soppressioni attive.
:::
...
**Stadio 3 — Governance Escalation:** 15 violazioni Z601 → $n_{\text{excess}} = 5$ → moltiplicatore $= 2^{5/5} = 2.0$ → deduzione $= 30 \times 2 = 60 \to \min(60, 25) = 25$. Bucket brand = 0.

**Stadio 4 — Gravity Cap:** $\text{cat\_pts}_{\text{brand}} = 0$ → $S_{\text{gravity}} = \min(42, 70) = 42$.

**Stadio 5 — Suppression Debt:** $n = 8$ soppressioni → flat-cost: $\omega_{\text{debt}} = 8$.

$$S_{\text{final}} = \max(0,\; 42 - 8) = \mathbf{34}$$

---

## Lettura dell'Output CLI {#cli-output}
...
**Quando scatta il Gravity Cap** (bucket Brand = 0), compare una riga extra tra
Σ Subtotal e Technical Debt:
