...
Ogni soppressione attiva è un'assunzione consapevole di responsabilità. Non è gratuita. La formula del debito è:
...
| :--- | :---: | :--- |
| $n \leq \text{cap}$ | 1 pt | Tariffa standard |
| $n > \text{cap}$ | 2 pts | Tier di escalation — ogni soppressione in eccesso costa il doppio |

:::tip Il Suppression Debt nella CLI
Esegui `zenzic score` per vedere la deduzione corrente del debito:

```text
Technical Debt (Suppressions): -8 pts
```

Esegui `zenzic check all --audit` per vedere qual sarebbe il punteggio senza alcuna soppressione.
:::
...
**Stadio 5 — Suppression Debt:** $n = 8 \leq 30$ → $\omega_{\text{debt}} = 8 \times 1 = 8$.
