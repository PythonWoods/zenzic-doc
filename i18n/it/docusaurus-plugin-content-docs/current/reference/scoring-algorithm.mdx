...
:::note Z106 è solo informativo
Z106 continua a essere emesso come diagnostica per i cicli di link, ma è escluso dalla tabella delle penalità e non contribuisce ad alcun bucket del DQS.
:::
...
Ogni soppressione attiva è un'assunzione consapevole di responsabilità. In v0.8.0, le soppressioni sono allowance-based: fino a `suppression_cap` sono debito autorizzato di governance e non riducono il punteggio. Solo le soppressioni oltre cap generano debito.

La formula del debito è:
...
- $\text{cap}$ = `suppression_cap` da `[governance]` in `zenzic.toml` (default: 30)
...
| :--- | :---: | :--- |
| $n \leq \text{cap}$ | 0 pt | Allowance gestita (tracciata, non penalizzata) |
| $n > \text{cap}$ | 1 pt (solo eccesso) | Il debito parte solo oltre il cap configurato |
...
Queste label descrivono la postura di governance. Sono complementari alla matematica del punteggio e aiutano i revisori a distinguere debito gestito da espansione di policy.
...
\max\!\left(0,\; S_{\text{gravity}} - \omega_{\text{debt}}\right) & \text{altrimenti}
\end{cases}
$$
...
**Stadio 5 — Suppression Debt:** $n = 8 \leq 30$ → $\omega_{\text{debt}} = 0$.
