...
**Cosa farai:** passare da una cartella di file Markdown a un report delZenzic in meno di
...
Il percorso più veloce per capire Zenzic è **vederlo in azione**.
`zenzic lab` esegue ogni scanner contro fixture pre-costruite: una vera violazione di sicurezza e un vero report pulito — in 60 secondi.
:::
...
**Demo credential scanner** — esegui l'atto 2 (demo credenziale esposta):
...
**Il Credential Scanner (Zenzic Audit Badge)** — ora esegui l'atto 0:

```bash
uvx zenzic lab 0
```

Il Zenzic Audit Badge: ogni controllo verde, exit 0. Il contrasto è la prova —
...
**Vuoi vedere il Credential Scanner in azione?**

```bash
uvx zenzic lab
```

Lancia il menu interattivo di tutti i 17 atti: credenziali esposte, link rotti, pagine orfane,
path traversal e il Zenzic Audit Badge. Zero installazione. Zero configurazione. Esperienza pura.

→ Scegli qualsiasi atto dal menu, oppure eseguine uno specifico: `uvx zenzic lab 2` (credential scanner), `uvx zenzic lab 0` (Il Sigillo).
:::
